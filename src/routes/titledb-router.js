...
td{padding:.55rem .75rem;border-bottom:1px solid #d0d7de;vertical-align:middle}
...
.btn{display:inline-flex;align-items:center;gap:.3rem;padding:.35rem .75rem;border:1px solid transparent;border-radius:6px;cursor:pointer;font-size:.8rem;font-weight:500;text-decoration:none;white-space:nowrap}
...
.modal{background:#fff;border-radius:8px;width:540px;max-height:90vh;overflow-y:auto;box-shadow:0 8px 32px rgba(0,0,0,.18)}
.modal-header{padding:1rem 1.25rem;border-bottom:1px solid #d0d7de;display:flex;align-items:center;justify-content:space-between}
...
<div class="container">
  ...
  <div id="panel-detected" class="panel active">
    <div class="toolbar">
      <h2>Games detected in your library</h2>
      <button class="btn btn-outline btn-sm" onclick="loadData()">↻ Refresh</button>
    </div>
    <table>
      <thead><tr><th>Title ID</th><th>Filename</th><th>Override</th><th></th></tr></thead>
      <tbody id="detected-body"><tr><td colspan="4" class="empty">Loading…</td></tr></tbody>
    </table>
  </div>

  <div id="panel-overrides" class="panel">
    <div class="toolbar">
      <h2>Server-side metadata overrides</h2>
      <button class="btn btn-primary btn-sm" onclick="openModal(null)">+ Add Override</button>
    </div>
    <table>
      <thead><tr><th>Title ID</th><th>Name</th><th>Publisher</th><th>Region</th><th>Version</th><th></th></tr></thead>
    ...
    <div class="modal-body">
      <form id="entry-form" onsubmit="saveEntry(event)">
        ...
        <div class="grid2">
          ...
          <div class="field">
            <label>Version</label>
            <input id="f-version" type="number" min="0" value="0" placeholder="0">
          </div>
          <div class="field">
            <label>Release Date <span style="color:#57606a;font-weight:400">(YYYYMMDD)</span></label>
          ...
          <div class="field">
            <label>Size <span style="color:#57606a;font-weight:400">(bytes)</span></label>
...
function renderDetected() {
  ...
  if (!detected.length) {
    tbody.innerHTML = '<tr><td colspan="4" class="empty">No game files found in the library folder.</td></tr>';
    return;
  }
  tbody.innerHTML = detected.map(({ titleId, filename }) => {
    const ov = overrides[titleId];
    const badge = ov
      ? \`<span class="badge badge-green">✓ \${ov.name || titleId}</span>\`
      : '<span class="badge badge-gray">none</span>';
    return \`<tr>
      <td><code>\${titleId}</code></td>
      <td style="max-width:360px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap" title="\${filename}">\${filename}</td>
      <td>\${badge}</td>
      <td style="text-align:right"><button class="btn btn-outline btn-sm" onclick="openModal('\${titleId}','\${escHtml(filename)}')">Edit</button></td>
    </tr>\`;
  }).join('');
}
...
function renderOverrides() {
  ...
  tbody.innerHTML = entries.map(e => \`<tr>
    ...
    <td>\${e.name || '<span style="color:#57606a">—</span>'}</td>
    <td>\${e.publisher || '<span style="color:#57606a">—</span>'}</td>
    <td>\${e.region || '<span style="color:#57606a">—</span>'}</td>
    <td>\${e.version ?? '<span style="color:#57606a">—</span>'}</td>
...
function openModal(titleId, filename) {
  ...
  const guessedName = filename ? filename.replace(/\\[.*$/, '').trim() : '';
  document.getElementById('modal-title').textContent = titleId ? 'Edit Override — ' + titleId : 'Add Override';
  ...
  document.getElementById('f-name').value        = e.name        ?? guessedName;
